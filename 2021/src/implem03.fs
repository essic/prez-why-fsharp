module Implem03
  ...
  let main _ =
    ...
    // collaborators : Option<Collaborator> list
    let collaborators = 
      [ createConsultant "Aly-Bocar" "Cisse" 32 [CSharp;FSharp]
        createManager "Luffy" "Dragon" 20 SGCIB 
        createConsultant "Nobody" "Ulysse" 15 [] 
        createManager "Kevin" "" 25 Natixis //Should fail since lastname is empty !
        createConsultant null "Something" 30 [Java] //Should fail since firstname is null !
        createConsultant "Gandalf" "The Gray" -200 [Finance] ]  // Should fail since age is negative !
    ...
    // (>>) : (a -> b) -> (b -> c) -> a -> c
    // ignore : 'T -> unit
    List.iter ( (fun maybeACollaborator -> Option.map (fun collaborator -> customPrint collaborator) maybeACollaborator) >> ignore) collaborators 
    0
